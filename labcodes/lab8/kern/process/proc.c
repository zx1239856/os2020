...
alloc_proc(void) {
     ...
     */
     //LAB5 2017011568 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized	
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2017011568 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2017011568 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2017011568
    //LAB8:EXERCISE2 2017011568  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    if(copy_mm(0, proc) != 0) goto bad_fork_cleanup_fs;
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2017011568  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    struct elfhdr fheader;
    struct elfhdr *elf = &fheader;
    if((ret = load_icode_read(fd, elf, sizeof(struct elfhdr), 0)) != 0) 
        goto bad_elf_cleanup_pgdir;
    //(3.2) get the entry of the program section headers of the bianry program (ELF format)
    struct proghdr pheader;
    struct proghdr *ph = &pheader;
